...
//			model.addAttribute("salesDate", salesDate);
	...
	public String admin_seat(Locale locale, HttpSession session, Model model) throws Exception {
	...
	@RequestMapping(value = "/seatEditProcess", method = RequestMethod.GET)
	public String seatEditProcess(HttpServletRequest request, HttpSession session, Model model) throws Exception {

		UserVO login_status = new UserVO();
		login_status = (UserVO) session.getAttribute("login");
		if (login_status.getUser_id().equals("admin")) {
			ComputerVO edit_computerVO = new ComputerVO();
			Calendar time = Calendar.getInstance();
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm");
			String date = format.format(time.getTime());
			model.addAttribute("date", date);
			...
			while (true) {
				String c_id = "c_id" + Integer.toString(i);
				String c_status = "c_status" + Integer.toString(i);
				i++;

				int computer_id = -1;
				int edit_computer_status = -1;
				if (request.getParameter(c_id) == null || request.getParameter(c_status) == null) {
					break;
				}

				if (!(request.getParameter(c_status).equals("정상") || request.getParameter(c_status).equals("고장"))) {
					continue;
				}

				computer_id = Integer.parseInt(request.getParameter(c_id));
				if (request.getParameter(c_status).equals("정상")) {
					edit_computer_status = 1;
				} else if (request.getParameter(c_status).equals("고장")) {
					edit_computer_status = 0;
				}

				edit_computerVO.setComputer_id(computer_id);
				edit_computerVO.setComputer_status(edit_computer_status);

				if (edit_computer_status == 1) {
					if (computer_service.updateComputer_status_t(edit_computerVO)) {
						System.out.println("좌석 상태 변경(정상)");
					}
				} else if (edit_computer_status == 0) {
					if (computer_service.updateComputer_status_f(edit_computerVO)) {
						System.out.println("좌석 상태 변경(고장)");
					}
				}
			}

			return "redirect:/admin_seat";
		}
		return "redirect:/user_main";
	}
}